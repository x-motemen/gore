...
func Run(extFiles, pkg string, autoImport bool) {
	s, err := NewSession()
	if err != nil {
		panic(err)
	}
	...
	if extFiles != "" {
		extFiles := strings.Split(extFiles, ",")
		s.includeFiles(extFiles)
	}

	if pkg != "" {
		err := s.includePackage(pkg)
		if err != nil {
			errorf("-pkg: %s", err)
			os.Exit(1)
		}
...
func (s *Session) Eval(in string, autoImport bool) error {
	debugf("eval >>> %q", in)
